...
int main() {
	...
	NK=((uint64_t)1<<6);
	NIV=((uint64_t)1<<36);
	printf("For the 5th linear approximation, we randomly generate 2^{%f} keys and 2^{%f} IVs, the experimental result is outputted as follows:\n\n",log2(NK),log2(NIV));
	...
	int indexOD12[] = {2*32+0, 2*32+2, 2*32+3, 2*32+4, 8*32+19, 8*32+20, 13*32+4};
